...
public class Trade {
    ...
    // Requires: t1 and t2 are different, getTradeSize() > 0, players in trade can only from 1 or 2 teams
    // Modifies: t1, t2
    ...
    public boolean conductTrade(Team t1, Team t2) {
        boolean successful = true;
        for (int i = 0; i < getTradedPlayers().size(); i++) {
            if (getTradedPlayers().get(i).getTeam().equals(t1.getTeamName())) {
                Player tradingPlayer = getTradedPlayers().get(i);
                t2.getPlayers().add(tradingPlayer);
                t1.getPlayers().remove(tradingPlayer);
            } else if (getTradedPlayers().get(i).getTeam().equals(t2.getTeamName())) {
                Player tradingPlayer = getTradedPlayers().get(i);
    ...
    // Modifies: this
    // Effects: add a player to the trade.
